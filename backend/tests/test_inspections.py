...
class TestInspectionApi:
    async def get_inspections_empty(self, client: AsyncClient):
    ...
    async def get_inspections_inspection_notfound(self, client: AsyncClient):
        response = await client.get("/api/inspections/9999/")
        assert response.status_code == 404

    async def test_get_inspecton(self, client: AsyncClient, inspection: Inspection):
    ...
    async def test_create_inspection_user_role(self, user_client: AsyncClient, gen_inspection_payload):
        payload = gen_inspection_payload()
        response = await user_client.post("/api/inspections/", json=payload)
        assert response.status_code == 403

    async def test_create_inspection_admin_role(self, admin_client: AsyncClient, gen_inspection_payload):
        payload = gen_inspection_payload()
        response = await admin_client.post("/api/inspections/", json=payload)
        assert response.status_code == 201

    async def test_create_inspection_superuser_role(self, superuser_client: AsyncClient, gen_inspection_payload):
        payload = gen_inspection_payload()
